...
class ActivityInsightOAStatusExporter < ActivityInsightExporter
  ...
  def export
    ...
    logger.info "Open access status export to Activity Insight started at #{DateTime.now}"
    not_exported_ids = []
    files.each do |file|
    ...
    logger.info "Open access status export to Activity Insight ended at #{DateTime.now}"
