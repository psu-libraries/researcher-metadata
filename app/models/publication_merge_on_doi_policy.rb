...
class PublicationMergeOnDoiPolicy
  ...
  def most_distant_value(attribute)
    [publication1.send(attribute), publication2.send(attribute)].reject(&:blank?).sort.first
  end
  ...
  def title
    ...
    if publication1.pure_import_identifiers.present?
      if secondary_title1.present? &&
          !title1&.downcase&.gsub(/[^a-z0-9]/, '').include?(secondary_title1&.downcase&.gsub(/[^a-z0-9]/, ''))
    ...
    if publication2.pure_import_identifiers.present?
      if secondary_title2.present? &&
          !title2&.downcase&.gsub(/[^a-z0-9]/, '').include?(secondary_title2&.downcase&.gsub(/[^a-z0-9]/, ''))
  ...
  def secondary_title
    ...
    if secondary_title1.present? &&
        !title&.downcase&.gsub(/[^a-z0-9]/, '').include?(secondary_title1&.downcase&.gsub(/[^a-z0-9]/, ''))
      return secondary_title1
    end

    if secondary_title2.present? &&
        !title&.downcase&.gsub(/[^a-z0-9]/, '').include?(secondary_title2&.downcase&.gsub(/[^a-z0-9]/, ''))
  ...
  def status
    [publication1.status, publication2.status].include?(Publication::PUBLISHED_STATUS) ?
  ...
  def publication_type
    [publication1, publication2].collect(&:publication_type_other?).count(true) == 1 ?
