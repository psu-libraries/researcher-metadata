...
class Publication < ApplicationRecord
  ...
  scope :unknown_version, -> { activity_insight_oa_publication.where(activity_insight_oa_files: { version: 'unknown' }).where(%{NOT EXISTS (SELECT * FROM activity_insight_oa_files WHERE activity_insight_oa_files.version IS NULL OR activity_insight_oa_files.version IN ('accepted', 'published'))}) }
