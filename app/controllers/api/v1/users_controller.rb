module API::V1
  class UsersController < APIController
    ...
    swagger_path '/v1/users/{webaccess_id}/grants' do
      operation :get do
        key :summary, "Retrieve a user's grants"
        key :description, 'Returns grant data for a user'
        key :operationId, 'findUserGrants'
        key :produces, [
          'application/json',
          'text/html'
        ]
        ...
        parameter do
          key :name, :webaccess_id
          key :in, :path
          key :description, 'Webaccess ID of user to retrieve grants'
          key :required, true
          key :type, :string
        end

        response 200 do
          key :description, 'user grants response'
          schema do
            key :required, [:data]
            property :data do
              key :type, :array
