...
class OrcidAccessTokensController < UserController
  ...
  def new
    if current_user.orcid_access_token.present?
      ...
      redirect_to "https://#{'sandbox.' unless Rails.env.production?}orcid.org/oauth/authorize?client_id=#{Rails.configuration.x.orcid['client_id']}&response_type=code&scope=/read-limited%20/activities/update%20/person/update&redirect_uri=#{CGI.escape(orcid_access_token_url)}"
    end
  end
