...
describe ScholarsphereFileHandler do
  subject(:file_handler) { described_class.new(publication, file_attributes) }
  ...
  describe '#version' do
    let(:exif_file_version) { double 'ExifFileVersionChecker', version: nil }

    before {
      allow(ExifFileVersionChecker)
        .to receive(:new)
    ...
    context 'when one file upload' do
      context 'uploaded file has no exif data' do
        it 'returns nil' do
          expect(file_handler.version).to be_nil
        end
      end
      ...
      context 'uploaded file is Accepted Manuscript' do
        ...
        it 'returns Accepted Manuscript' do
          expect(file_handler.version).to eq I18n.t('file_versions.accepted_version')
        end
      end

      context 'uploaded file is Final Published Version' do
        let(:exif_file_version) { double 'ExifFileVersionChecker', version: I18n.t('file_versions.published_version') }

        it 'returns Final Published Version' do
          expect(file_handler.version).to eq I18n.t('file_versions.published_version')
        end
      end
    end
    ...
    context 'when multiple file uploads' do
      ...
      let(:exif_file1_version) { double 'ExifFileVersionChecker', version: version1 }
      let(:exif_file2_version) { double 'ExifFileVersionChecker', version: version2 }
      let(:exif_file3_version) { double 'ExifFileVersionChecker', version: version3 }
      ...
      before {
        allow(ExifFileVersionChecker).to receive(:new).with(options1).and_return exif_file1_version
        allow(ExifFileVersionChecker).to receive(:new).with(options2).and_return exif_file2_version
        allow(ExifFileVersionChecker).to receive(:new).with(options3).and_return exif_file3_version
      }
  ...
  describe '#cache_files' do
    ...
    before do
      allow(ExifFileVersionChecker).to receive(:new).and_return exif_file_version
    ...
    it 'caches uploaded files and returns cache info' do
      expect(file_handler.cache_files).to eq [
  ...
  describe 'validating file upload' do
    context 'when there is no file upload' do
      ...
      it 'does not validate' do
        ...
        expect(file_handler.errors[:file_uploads]).to include I18n.t('models.scholarsphere_work_deposit.validation_errors.file_upload_presence')
      end
    end
