...
RSpec.describe DeputyAssignmentComponent, type: :component do
  ...
  context 'when current_user is the primary' do
    ...
    it 'shows the name and access id of the deputy' do
      expect(rendered_content).to have_css('.deputy-assignment__name', text: 'Deputy User dep456')
    end
    ...
    context 'when the deputy_assignment is pending confirmation' do
      ...
      it 'shows a "pending" message' do
        expect(rendered_content).to have_text(I18n.t!('view_component.deputy_assignment_component.pending_as_primary'))
      end

      it 'shows the delete button' do
        expect(rendered_content).to have_button(I18n.t!('view_component.deputy_assignment_component.delete_as_primary'), class: 'btn-outline-danger')
      end
    end
  ...
  context 'when current_user is the deputy' do
    ...
    it 'shows the name and access id of the primary' do
      expect(rendered_content).to have_css('.deputy-assignment__name', text: 'Primary User pri123')
    end
    ...
    context 'when the deputy_assignment is pending confirmation' do
      ...
      it 'adds an action-required class' do
        expect(rendered_content).to have_css('.deputy-assignment--action-required')
      end

      it 'shows an action-required message' do
        expect(rendered_content).to have_text(I18n.t!('view_component.deputy_assignment_component.pending_as_deputy'))
      end

      it 'shows a button to accept the DeputyAssignment' do
        expect(rendered_content).to have_button(I18n.t!('view_component.deputy_assignment_component.accept'))
      end

      it 'shows a special delete button' do
        expect(rendered_content).to have_button(I18n.t!('view_component.deputy_assignment_component.delete_as_deputy_unconfirmed'), class: 'btn-outline-secondary')
      end
    end
    ...
    context 'when the deputy_assignment has been confirmed' do
      ...
      it 'shows the delete button' do
        expect(rendered_content).to have_button(I18n.t!('view_component.deputy_assignment_component.delete_as_deputy'), class: 'btn-outline-danger')
      end
    end
  end
end
