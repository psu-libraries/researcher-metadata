...
RSpec.describe 'api/v1/users' do
        ...
        schema type: :object, properties: {
          data: {
            ...
            items: {
              ...
              properties: {
                id: { type: :string, example: '123', description: 'The ID of the object' },
                type: { type: :string, example: 'performance', description: 'The type of the object' },
                attributes: {
                  type: :object,
                  required: [:title, :activity_insight_id],
                  properties: {
                    title: { type: :string, example: 'Example Performance', description: 'The title of the performance' },
                    activity_insight_id: { type: :integer, example: '1234567890', description: "The unique identifier for the performance's corresponding record in the Activity Insight database" },
                    performance_type: { type: [:string, :null], example: 'Film - Documentary', description: 'The type of performance' },
                    sponsor: { type: [:string, :null], example: 'Penn State', description: 'The organization that is sponsoring this performance' },
                    description: { type: [:string, :null], example: 'This is a unique performance, performed for specific reasons', description: 'Any further detail describing the performance' },
                    group_name: { type: [:string, :null], example: 'Penn State Performers', description: 'The name of the performing group' },
                    location: { type: [:string, :null], example: 'State College, PA', description: 'Country, State, City, theatre, etc. where the performance took place' },
                    delivery_type: { type: [:string, :null], example: 'Competition', description: 'Audition, commission, competition, or invitation' },
                    scope: { type: [:string, :null], example: 'Local', description: 'International, national, regional, state, local' },
                    start_on: { type: [:string, :null], example: '12-01-2015', description: 'The date that the performance started on' },
                    end_on: { type: [:string, :null], example: '12-31-2015', description: 'The date that the performance ended on' },
                    user_performances: {
                      type: :array,
                      items: {
                        type: :object,
                        properties: {
                          first_name: { type: [:string, :null], example: 'Billy', description: 'The first name of a contributor' },
                          last_name: { type: [:string, :null], example: 'Bob', description: 'The last name of a contributor' },
                          contribution: { type: [:string, :null], example: 'Performer', description: 'The contributor\'s role/contribution to the performance' },
                          student_level: { type: [:string, :null], example: 'Graduate', description: 'Undergraduate or graduate' },
                          role_other: { type: [:string, :null], example: 'Director', description: 'Role not listedin "contribution" drop-down' }
                        }
                      }
                    },
                    performance_screenings: {
                      ...
                      items: {
                        type: :object,
                        properties: {
                          name: { type: [:string, :null], example: 'Film Festival', description: 'Name of the venue for the screening' },
                          location: { type: [:string, :null], example: 'State College, PA', description: 'Country, State, City, where the screening took place' },
                          screening_type: { type: [:string, :null], example: 'DVD Distribution', description: 'Type of screening/exhibition' }
                        }
                      }
                    },
                    profile_preferences: {
  ...
  path '/v1/users/{webaccess_id}/publications' do
    get 'Retrieve a user\'s publications' do
      ...
      parameter name: :webaccess_id, in: :path, description: 'Webaccess ID of user to retrieve publications', required: true, type: :string
      parameter name: :start_year, in: :query, description: 'Beginning of publication year range', required: false, type: :integer, format: :int32
      parameter name: :end_year, in: :query, description: 'End of publication year range', required: false, type: :integer, format: :int32
      ...
      response 200, 'user profile response' do
        schema type: :object, properties: {
          data: {
            type: :object,
            properties: {
              id: { type: :string, example: '123', description: 'The ID of the user' },
              type: { type: :string, example: 'user_profile', description: 'The type of the object' },
              attributes: {
                type: :object,
                properties: {
                  name: { type: :string, example: 'Example User', description: 'The full name of the user' },
                  organization_name: { type: :string, example: 'College of Engineering', description: "The name of the user's primary organization" },
                  title: { type: :string, example: 'Professor', description: "The title of the user's position" },
                  email: { type: :string, example: 'abc123@psu.edu', description: "The user's email address" },
                  office_location: { type: :string, example: '101 Chemistry Building', description: "The room number and building where the user's office is located" },
                  office_phone_number: { type: :string, example: '(555) 555-555', description: "The telephone number for the user's office" },
                  personal_website: { type: :string, example: 'mysite.org', description: "The domain or URL for the user's personal website" },
                  total_scopus_citations: { type: :integer, example: 76, description: "The total number of times that all of the user's publications have been cited as recorded in Scopus (Pure)" },
                  scopus_h_index: { type: :integer, example: 24, description: "The user's H-Index value in Scopus (Pure)" },
                  pure_profile_url: { type: :string, example: 'https://pennstate.pure.elsevier.com/en/persons/abc123-def456', description: "The URL for the user's profile page on the Penn State Pure website" },
                  orcid_identifier: { type: :string, example: 'https://orcid.org/0000-0000-0000-0000', description: "The URL for the user's ORCID ID" },
                  bio: { type: :string, example: 'Some biographical information about this user', description: 'A brief biography of the user' },
                  teaching_interests: { type: :string, example: 'Computer Science, Information Technology', description: "A description of the user's teaching interests" },
                  research_interests: { type: :string, example: 'Quantum Computing, Encryption', description: "A description of the user's research interests" },
                  publications: { type: :array, items: { type: :string, example: '<span class="publication-title">My Publication</span>, <span class="journal-name">Journal of Medicine</span>, 2010', description: 'A string of HTML describing a journal article' } },
                  other_publications: {
                    type: :object,
                    properties: {
                      Books: { type: :array, items: { type: :string, example: '<span class="publication-title">My Book</span>, <span class="journal-name">Journal of Science</span>, 2012' } },
                      Letters: { type: :array, items: { type: :string, example: '<span class="publication-title">My Letter</span>, <span class="journal-name">Journal of Physics</span>, 2011' } }
                    },
  ...
  path '/v1/users/publications' do
    post 'Retrieve publications for a group of users' do
      ...
      parameter name: :start_year, in: :query, description: 'Beginning of publication year range', required: false, type: :integer, format: :int32
      parameter name: :end_year, in: :query, description: 'End of publication year range', required: false, type: :integer, format: :int32
      parameter name: :order_first_by, in: :query, description: 'Orders publications returned', required: false, schema: { type: :string, enum: [
        :citation_count_desc,
        :publication_date_asc,
        :publication_date_desc,
        :title_asc
      ] }
      parameter name: :order_second_by, in: :query, description: 'Orders publications returned', required: false, schema: { type: :string, enum: [
        :citation_count_desc,
        :publication_date_asc,
        :publication_date_desc,
        :title_asc
      ] }
      parameter name: :limit, in: :query, description: 'Max number publications to return for each user', required: false, type: :integer, format: :int32
      parameter name: :webaccess_ids, in: :body, description: 'Webaccess IDs of users to retrieve publications', required: true, schema: {
        type: :array,
        items: {
          type: :string
        }
